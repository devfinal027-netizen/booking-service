...
let payoutsCtrl = null;
try {
  payoutsCtrl = require('../../controllers/reports/payouts.controller');
} catch (e) {
  // Graceful fallback if controller file isn't present in deployed build
  console.warn('[analytics.routes] payouts.controller not found; payouts routes disabled');
}
...
// Payouts management - Admin only (register only if controller is available)
if (payoutsCtrl) {
